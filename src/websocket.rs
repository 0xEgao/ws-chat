...
//this will handle ws socket connection and server to this server
pub async fn handle_connection(
    ...
    write
        .send(Message::Text(rooms_message.into()))
        .await
        .expect("Failed to send room list");
        ...
        match message {
            Ok(Message::Text(text)) => {
                if text.starts_with("CREATE_ROOM:") {
                    ...
                    channel_manager
                        .remove_sender_from_channel(room_name.to_string(), tx.clone())
                        .await;
                    ...
                    if parts.len() == 3 {
                        ...
                        let formatted_message =
                            format!("MSG:{}:{}:{}", room_name, sender_name, message_text);
                        channel_manager
                            .broadcast_message(
                                room_name,
                                sender_name.clone(),
                                message_text.clone(),
                                Message::Text(formatted_message.into()),
                            )
                            .await;
                    }
                }
